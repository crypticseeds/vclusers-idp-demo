...
Run the `install_script.sh` to install ArgoCD, Crossplane and Komoplane. It will also get Backstage ready. You'll need to create a few more configurations before running Backstage.
...
**Komoplane** is a user interface (UI) designed for **Crossplane**, offering a visual representation of Crossplane resources such as **Custom Resource Definitions (CRDs)**, **Composite Resource Definitions (XRDs)**, and **claims**. It simplifies the management of these resources by providing an intuitive interface for viewing their status, relationships, and configurations. Additionally, Komoplane facilitates quick and efficient troubleshooting by highlighting issues and enabling streamlined navigation through the Crossplane ecosystem. 

You can port forward komoplane service like this:

```bash
kubectl port-forward svc/komoplane 8090:8090 --address 0.0.0.0
```
...
You can create the Kubernetes secret from the `aws_credentials.conf` file using `kubectl`. 

Run the following command:

```sh
kubectl create secret generic aws-creds --from-file=creds=./aws_credentials.conf -n crossplane-system
```

This command does the following:
- `create secret generic aws-creds` tells Kubernetes to create a new generic secret named `aws-creds`.
- `--from-file=creds=./aws_credentials.conf` adds the content of your `aws_credentials.conf` file to the secret under the key `creds`.
...
```bash
./backstage/my-backstage-app/create_k8s_secrets_for_backstage.sh
```
3. Create this secret with:
```bash
kubectl apply -f ./my-backstage-app/my-backstage-secrets.yaml
```
...
kubectl create secret generic google-creds \
  --from-file=google-creds.json=./backstage/my-backstage-app/packages/backend/google-creds.json \
  --namespace backstage
```
...
```bash
helm upgrade --install backstage backstage/backstage --namespace backstage -f ./my-backstage-app/values.yaml --set backstage.image.tag=v0.0.2
```
...
```bash
kubectl port-forward -n backstage service/backstage 7007:7007 --address 0.0.0.0
```
